...
@router.get("/{schedule_id}", response_model=MedicScheduleSchema)
def get_schedule(schedule_id: int, db: Session = Depends(get_db)):
    return get_schedule_by_id(db, schedule_id)

#GET Traer horarios del medico
@router.get("/by-medic/{medic_id}", response_model=List[MedicScheduleSchema])
def get_schedules_by_medic(medic_id: int, db: Session = Depends(get_db)):
...
def update_medic_schedule(schedule_id:int,schedule_update:MedicScheduleUpdate,db: Session=Depends(get_db)):
    update = update_schedule(db, schedule_id, schedule_update)
    if not update:

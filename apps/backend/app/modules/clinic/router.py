...
def get_clinic_route(clinic_id:int, db: Session=Depends(get_db)):
...
def create_clinic_route(clinic_data:ClinicCreate, db:Session=Depends(get_db)):
    clinic = create_clinic(db,clinic_data)
    return clinic

# PUT: Actualizar un usuario
@router.put("/{clinic_id}", response_model=ClinicSchema)
def update_clinic_route(clinic_id: int, updated_user: ClinicUpdate, db: Session = Depends(get_db)):
...
def delete_clinic_route(clinic_id: int, db: Session = Depends(get_db)):
